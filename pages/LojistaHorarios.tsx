...
export const LojistaHorarios = () => {
  ...
  const [schedule, setSchedule] = useState<DiaFuncionamento[]>(minhaLoja.horarios || []);
  ...
  // Inicializa o schedule se estiver vazio
  useEffect(() => {
    if (schedule.length === 0) {
      const initialSchedule = DAYS_OF_WEEK.map(dia => ({
        dia,
        ativo: true,
