  ...
  return (
    <div className="min-h-screen bg-[#f1f4f9] flex flex-col font-sans">
      ...
      <header className="bg-white px-8 py-6 flex items-center justify-between border-b border-gray-100 sticky top-0 z-50">
        ...
        <button onClick={() => navigate('/')} className="text-xs font-bold text-gray-400 hover:text-gray-600 transition-colors uppercase tracking-widest">Sair</button>
      </header>
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-6 md:p-12 relative overflow-hidden">
        ...
        {step === 1 && (
          <div className="w-full max-w-3xl bg-white rounded-[2.5rem] shadow-2xl border border-gray-100 overflow-hidden animate-fade-in">
            <div className="p-12 md:p-16">
              ...
              <div className="space-y-2 mb-10">
                <label className="text-xs font-bold text-gray-500 uppercase tracking-widest">Endereço Completo (Preenchimento Manual)</label>
                <input 
                  type="text" 
                ...
                />
                <p className="text-[10px] text-gray-400 font-bold mt-1">Digite o endereço completo da sua loja. Não utilizamos mapa automático nesta etapa.</p>
              </div>
          ...
          <div className="w-full max-w-7xl flex flex-col lg:flex-row gap-12 animate-fade-in">
            <div className="flex-1 space-y-12">
              <div>
                ...
                <p className="text-xl text-gray-400 font-medium">Essas imagens aparecerão para seus clientes no app.</p>
              </div>

              <div className="bg-white rounded-[2.5rem] p-10 shadow-sm border border-gray-100">
                <h3 className="text-xl font-black text-gray-900 flex items-center gap-4 mb-8">Logotipo da Loja</h3>
                <div className="flex flex-col md:flex-row items-center gap-10">
                   <input type="file" ref={logoInputRef} className="hidden" accept="image/*" onChange={(e) => handleFileChange(e, 'logo')} />
                   <div onClick={() => logoInputRef.current?.click()} className="w-48 h-48 bg-gray-50 rounded-[2.5rem] border-2 border-dashed border-gray-200 flex flex-col items-center justify-center text-gray-300 hover:border-emerald-500 hover:text-emerald-500 cursor-pointer transition-all overflow-hidden relative">
                      {form.logo ? <img src={form.logo} alt="Logo" className="w-full h-full object-cover" /> : <span className="text-xs font-black uppercase tracking-widest">Upload</span>}
                   </div>
              ...
              <div className="bg-white rounded-[2.5rem] p-10 shadow-sm border border-gray-100">
                <h3 className="text-xl font-black text-gray-900 flex items-center gap-4 mb-8">Capa / Banner</h3>
                <input type="file" ref={bannerInputRef} className="hidden" accept="image/*" onChange={(e) => handleFileChange(e, 'banner')} />
                <div onClick={() => bannerInputRef.current?.click()} className="w-full h-48 bg-gray-50 rounded-[2rem] border-2 border-dashed border-gray-200 flex flex-col items-center justify-center text-gray-300 hover:border-emerald-500 cursor-pointer overflow-hidden relative">
                  {form.banner ? <img src={form.banner} alt="Banner" className="w-full h-full object-cover" /> : <p className="text-xs font-black uppercase tracking-widest">Arraste a imagem</p>}
                </div>
              </div>

              <div>
                <h3 className="text-3xl font-black text-gray-900 mb-4 tracking-tighter">Qual o seu ramo?</h3>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                  {['Restaurante', 'Mercado', 'Lanches', 'Pet Shop', 'Farmácia', 'Outros'].map(cat => (
                    <button key={cat} onClick={() => setForm({...form, ramo: cat})} className={`p-8 rounded-[2rem] flex flex-col items-center justify-center text-center transition-all border-4 ${form.ramo === cat ? 'bg-emerald-50 border-emerald-500 shadow-xl' : 'bg-white border-transparent'}`}>
                      <span className="font-black text-gray-800 text-lg">{cat}</span>
                    </button>
                  ))}
                </div>
              </div>
        ...
        {step === 3 && (
          <div className="w-full max-w-5xl animate-fade-in">
             <div className="text-center mb-12">
                <h2 className="text-5xl font-black text-gray-900 mb-4 tracking-tighter">Escolha seu plano</h2>
                <p className="text-xl text-gray-400 font-medium">Comece com 7 dias grátis. Cancele quando quiser.</p>
             </div>
             ...
             <div className="bg-white p-8 rounded-[2rem] max-w-2xl mx-auto shadow-sm border border-gray-100">
                ...
                <div className="space-y-4">
                   <input type="text" placeholder="Número do Cartão" className="w-full bg-gray-50 border-none rounded-xl p-4 font-bold text-gray-700 outline-none" />
                   <div className="flex gap-4">
                      <input type="text" placeholder="Validade (MM/AA)" className="flex-1 bg-gray-50 border-none rounded-xl p-4 font-bold text-gray-700 outline-none" />
                      <input type="text" placeholder="CVV" className="flex-1 bg-gray-50 border-none rounded-xl p-4 font-bold text-gray-700 outline-none" />
                   </div>
                   <input type="text" placeholder="Nome no Cartão" className="w-full bg-gray-50 border-none rounded-xl p-4 font-bold text-gray-700 outline-none" />
                </div>
        ...
        <div className="fixed bottom-0 left-0 w-full p-8 md:p-12 pointer-events-none flex justify-center z-50">
          <div className="w-full max-w-7xl flex justify-between items-center pointer-events-auto">
            <button onClick={handleBack} className="bg-white border-2 border-gray-100 text-gray-400 px-10 py-5 rounded-[2rem] font-black text-xl hover:border-emerald-500 hover:text-emerald-700 transition-all shadow-xl active:scale-95">Voltar</button>
            <button onClick={handleNext} className="bg-emerald-600 text-white px-12 py-5 rounded-[2rem] font-black text-xl shadow-2xl flex items-center gap-4 hover:bg-emerald-500 transition-all transform hover:-translate-y-1 active:scale-95">
