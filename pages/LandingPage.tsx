...
export const LandingPage = () => {
  ...
  return (
    <div className="min-h-screen bg-white">
      <nav className="p-8 max-w-7xl mx-auto flex justify-between items-center bg-white/80 backdrop-blur-md sticky top-0 z-50">
        <h1 className="text-3xl font-black text-emerald-700 tracking-tighter">PEDE MAIS</h1>
        <div className="flex items-center gap-8 font-black uppercase text-[10px] tracking-widest">
          <a href="#calculadora" className="text-gray-400 hover:text-emerald-600 transition-colors">Calculadora</a>
          <a href="#cases" className="text-gray-400 hover:text-emerald-600 transition-colors">Cases</a>
          <Link to="/login" className="bg-emerald-600 text-white px-8 py-3 rounded-2xl shadow-lg shadow-emerald-100 hover:scale-105 transition-all">ENTRAR</Link>
        </div>
      </nav>

      <section className="pt-24 pb-40 px-6 text-center max-w-6xl mx-auto">
        <div className="inline-block bg-emerald-50 text-emerald-700 px-6 py-2 rounded-full text-[10px] font-black uppercase tracking-widest mb-10 animate-pulse">
          🚀 O MELHOR SAAS PARA DELIVERY DIRETO
        </div>
        <h1 className="text-7xl md:text-9xl font-black text-gray-900 leading-[0.85] tracking-tighter mb-12">
        ...
        <div className="flex flex-col md:flex-row justify-center gap-6">
          <button 
            ...
            className="bg-gray-900 text-white px-12 py-6 rounded-[2.5rem] font-black text-2xl shadow-2xl hover:bg-emerald-600 transition-all hover:scale-105"
          >
            COMEÇAR AGORA
          </button>
          <button 
            onClick={() => navigate('/onboarding')}
            className="bg-white border-2 border-gray-100 text-gray-900 px-12 py-6 rounded-[2.5rem] font-black text-2xl hover:border-emerald-500 transition-all"
          >
            VER DEMONSTRAÇÃO
          </button>
        </div>
      ...
      <section id="calculadora" className="bg-gray-50 py-40">
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid lg:grid-cols-2 gap-24 items-center">
            ...
            <div className="bg-emerald-900 text-white rounded-[3.5rem] p-16 shadow-3xl text-center relative overflow-hidden">
               <div className="absolute top-0 right-0 w-64 h-64 bg-emerald-500 rounded-full blur-[120px] opacity-20" />
               <p className="text-emerald-400 font-black uppercase tracking-widest text-xs mb-6">ECONOMIA MENSAL POTENCIAL</p>
               <h3 className="text-7xl md:text-8xl font-black mb-10 tracking-tighter">{formatCurrency(roiExtra)}</h3>
               ...
               <button 
                onClick={() => navigate('/onboarding')}
                className="mt-12 w-full bg-emerald-500 hover:bg-emerald-400 text-white py-6 rounded-[2rem] font-black text-xl shadow-xl transition-all"
               >
      ...
      <section id="cases" className="py-40 overflow-hidden">
        <div className="max-w-7xl mx-auto px-6 mb-20 text-center">
          <h2 className="text-5xl font-black text-gray-900 tracking-tighter">Quem usa, <span className="text-emerald-600">vende mais.</span></h2>
        </div>
        <div className="flex gap-8 px-6 animate-scroll whitespace-nowrap">
          {[...cases, ...cases].map((c, i) => (
            <div key={i} className="inline-block bg-white border border-gray-100 p-10 rounded-[2.5rem] shadow-sm min-w-[350px]">
              <div className="text-5xl mb-6">{c.logo}</div>
              <h4 className="text-2xl font-black text-gray-900 mb-2">{c.nome}</h4>
              <p className="text-gray-400 font-bold mb-6">{c.cidade}</p>
              <div className="bg-emerald-50 text-emerald-700 p-6 rounded-2xl font-black">
                Crescimento de <span className="text-3xl block">{c.crescimento}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="bg-gray-900 py-32 px-6">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-5xl font-black text-white tracking-tighter mb-8">Experimente grátis por 7 dias.</h2>
          <p className="text-gray-400 text-xl mb-12">Sem cartão de crédito, sem burocracia. Cadastre seu e-mail e comece a vender.</p>
          <form className="flex flex-col md:flex-row gap-4" onSubmit={(e) => { e.preventDefault(); navigate('/onboarding'); }}>
            <input 
              ...
              className="flex-1 bg-white/10 border-2 border-white/10 rounded-2xl px-8 py-5 text-white font-bold focus:border-emerald-500 outline-none transition-all"
            />
            <button className="bg-emerald-600 hover:bg-emerald-500 text-white px-12 py-5 rounded-2xl font-black text-xl shadow-xl transition-all">
              CRIAR MINHA LOJA
            </button>
          </form>
        </div>
      ...
      <footer className="py-20 bg-white border-t border-gray-50 px-6">
        <div className="max-w-7xl mx-auto grid md:grid-cols-4 gap-12">
          <div className="col-span-2">
            ...
            <p className="text-gray-400 font-medium text-lg max-w-sm">A maior plataforma de canais diretos do Brasil. Tecnologia de ponta para o pequeno e médio varejista.</p>
          </div>
          <div>
          ...
          <div className="flex flex-col items-end justify-between">
            <a href="https://wa.me/5511999999999?text=Quero%20vender%20no%20Pede%20Mais" target="_blank" className="flex items-center gap-4 bg-[#25D366] text-white px-8 py-4 rounded-2xl font-black shadow-lg hover:scale-105 transition-all">
